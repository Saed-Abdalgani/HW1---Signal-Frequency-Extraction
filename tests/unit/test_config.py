...
Detailed validation tests are in:
- test_config_setup.py (setup schema and ranges)
- test_config_rate_limits.py (rate limits validation)
- test_config_logging.py (logging validation)
"""
...
class TestLoadJson:
    """JSON config loading and caching tests."""
    ...
    def test_get_rate_limits_basic(self, tmp_config_dir) -> None:
        """get_rate_limits returns dict with 'services' key."""
        rl = get_rate_limits()
        assert "services" in rl

    def test_get_logging_config_basic(self, tmp_config_dir) -> None:
